...
class MessageSend:
    def __init__(self):
        ...
        self.register("telegram_token", self.telegram)
    ...
    def telegram(self, telegram_token, title, content):
        bot_token, chat_id  = telegram_token.split(',')
        assert type(bot_token) == str and type(chat_id) == str, "Wrong types for Telegram token or chat_id."
        ...
        resp_json = resp.json()
        if resp_json["ok"]:
            print(f"[Telegram] Send message to Telegram successfully.")
        if not resp_json["ok"]:
